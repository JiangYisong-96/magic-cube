...
public class MagicDynamicDataSource {
  ...
  public void put(String id, String dataSourceKey, String datasourceName, DataSource dataSource,
    ...
    logger.info("注册database数据源：{}",
        StringUtils.isNotBlank(dataSourceKey) ? dataSourceKey : "default");
    DataSourceNode node = this.dataSourceMap.put(dataSourceKey,
    ...
    if (id != null) {
      ...
      this.dataSourceMap.entrySet().stream()
          .filter(it -> id.equals(it.getValue().getId()) && !finalDataSourceKey.equals(
              it.getValue().getKey()))
          .findFirst()
          .ifPresent(it -> {
