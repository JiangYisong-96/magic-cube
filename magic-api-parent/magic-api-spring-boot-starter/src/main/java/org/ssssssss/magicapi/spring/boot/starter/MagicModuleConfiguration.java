...
import org.ssssssss.magicapi.modules.thread.ThreadModule;
import org.ssssssss.magicapi.rabbitmq.RabbitmqModule;
...
public class MagicModuleConfiguration {
  ...
  public PageProvider pageProvider() {
    ...
    logger.info(
        ...
        pageConfig.getPage(), pageConfig.getSize(), pageConfig.getDefaultPage(),
        pageConfig.getDefaultSize(), pageConfig.getMaxPageSize());
    return new DefaultPageProvider(pageConfig.getPage(), pageConfig.getSize(),
        pageConfig.getDefaultPage(), pageConfig.getDefaultSize(), pageConfig.getMaxPageSize());
  }
  ...
  public SqlCache sqlCache() {
    ...
    logger.info("未找到SQL缓存实现，采用默认缓存实现(LRU+TTL)，缓存配置:(容量={},TTL={})",
  ...
  public SQLModule magicSqlModule(MagicDynamicDataSource dynamicDataSource,
    ...
    List<SQLInterceptor> sqlInterceptors = sqlInterceptorsProvider.getIfAvailable(
        ArrayList::new);
  ...
  @Bean
  public RabbitmqModule rabbitmqModule(MagicRabbitMqSource magicRabbitMqSource) {
    RabbitmqModule rabbitmqModule = new RabbitmqModule(magicRabbitMqSource);
    if (magicRabbitMqSource.isEmpty()) {
      logger.info("There is no rabbitmq source configured.");
    }
  ...
  @Bean
  @ConditionalOnMissingBean(TestModule.class)
  public TestModule magicTestModule() {
   ...
   */
  @Bean
  @ConditionalOnMissingBean(ThreadModule.class)
  public ThreadModule threadFunction() {
